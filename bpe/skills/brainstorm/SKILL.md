...
4. Ask the user ONE question with four parts, each carrying a proposed default so a short answer closes all four:
   ...
   - Vendoring: the shared verdict and the proposed vendor set. When the shared verdict is a fence-sitter (see Vendoring step 1), say so and let the user call it; otherwise present it as a default. Omit this part entirely when the repo is private or the vendor set is empty (no source in the session governs an artifact class the project produces); never ask the user to confirm vendoring nothing.
...
2. Pick the artifact classes the project produces (Python code, prose docs, IaC, slides, and so on) from the Q&A and the repo, then the sources that govern each class: skills from the session list, user-level rules in `~/.claude/rules/`, and any file `~/.claude/CLAUDE.md` imports with `@path` (Claude Code expands those at launch, so they are already part of the user's standing instructions). A source whose description or first line says `never vendored` is excluded without judgment. Sort the rest into a bucket:
...
3. For each source in the vendor set, obtain its text: invoke a skill through the Skill tool, or Read a user-level rules file. If the source has a section headed "Hard Rules" (matched case-insensitively; an equivalent explicit rules block also counts), copy that section verbatim; do not paraphrase, reorder, or trim it. If it has no such section, extract the must/never rules into a bullet list, show the extract to the user before writing it, and suggest adding a "Hard Rules" block to the source so the next resync is a verbatim copy.
